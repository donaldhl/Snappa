...
# Minimum tosses (MP) to appear on leaderboard
MIN_TOSSES = 100

# Team assignments — matches SNER sheet section structure
# Cream Team = rows 3-17, Dumplings = rows 20-29, FA = rows 32-38
TEAM_MAP = {
    # Cream Team
    ...
    "Sungwon": "Dumplings",
    # FA = everyone else (Germaine, Jen S, Eric S, Jake, Sam, Kate, Su)
}

# Free Agents shown in the FA Bids tab — full undrafted list from SNER sheet rows 32-38
FA_NAMES = ["Germaine", "Jen S", "Eric S", "Jake", "Sam", "Kate", "Su"]
...
def compute_player_stats(match_rows):
    ...
    for p in players:
        ...
        # Adjusted stats = raw * pace_adj (matching sheet col E,F,G)
    ...
    qualifiers = [p for p in players if p["mp"] >= MIN_TOSSES]
...
def compute_team_stats(players, match_rows):
    """
    Compute team stats directly from Match History columns,
    matching the sheet's SUMIF/COUNTIF formulas in cols AA-AK.
    Team column values: 'Cream', 'Dumplings', 'Free Agent'
    """
    teams = {
    ...
    # Per-row stats (SUMIF on Team column)
    for row in match_rows:
    ...
    # Refs = COUNTIF(Ref Team col, team) — col F "Ref Team"
    for row in match_rows:
        ref_team = str(row.get("Ref Team", "")).strip()
        if ref_team in teams:
            teams[ref_team]["refs"] += 1

    # Shotguns = COUNTIF(Team Shogun col, team) — deduplicated by game
    seen_sg = set()
    for row in match_rows:
    ...
    # Wins = COUNTIF(Team Win col, team) — unique per game
    seen_games = {}
    for row in match_rows:
        gnum = str(row.get("Game", "")).strip()
    ...
    # Avg SNER per team (from player list, qualified players only)
    for team_key in teams:
        t_players = [p for p in players if p["team"] == team_key and p["mp"] >= MIN_TOSSES]
        if t_players:
            teams[team_key]["avg_sner"]    = round(sum(p["qSNER"] for p in t_players) / len(t_players), 2)
            teams[team_key]["avg_fg_pct"]  = round(sum(p["fg_pct"] for p in t_players if p["fg_pct"] is not None) /
                                                    max(1, sum(1 for p in t_players if p["fg_pct"] is not None)), 4)
            teams[team_key]["avg_tfg_pct"] = round(sum(p["tfg_pct"] for p in t_players if p["tfg_pct"] is not None) /
...
def inject_data(template_html, players, teams, build_date):
    # 1. Rebuild JS player array — use display team labels matching template
    ...
    # Only show qualified players (>= 100 tosses) on the dashboard
    qualified_players = [p for p in players if p["mp"] >= MIN_TOSSES]
    ...
    # 2. Update FA_NAMES list in JS to match current undrafted FA list
    fa_js = "var FA_NAMES=" + json.dumps(FA_NAMES) + ";"
    html = re.sub(r'var FA_NAMES=\[.*?\];', fa_js, html)

    # 3. Rename "Monthly Stats" tab to "Live Stats"
    html = html.replace('>Qualifiers<', '>Live Stats<')
    html = html.replace("st('monthly')", "st('monthly')")  # keep id unchanged

    # 4. Update team stats table tbody
    c  = teams["Cream"]
    d  = teams["Dumplings"]
    fa = teams["Free Agent"]
    ...
    # 5. Update FA bids table headers to match FA_NAMES
    fa_headers = "".join(
    ...
    # 6. Update team banner
    cw2 = c["wins"]; dw2 = d["wins"]
    tot = cw2 + dw2
    cp = f"{round(cw2/tot*100,1)}%" if tot else "—"
    dp = f"{round(dw2/tot*100,1)}%" if tot else "—"
    headline = (
        f"<strong>Head-to-head:</strong> Cream Team won {cw2}–{dw2} "
