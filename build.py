...
TEAM_MAP = {
    ...
    "Sungwon": "Dumplings",
    # Free Agents (everyone else defaults to this)
}
...
def fetch_sheet_csv(sheet_name):
    """Fetch a Google Sheet tab as a list of dicts (using published CSV URL)."""
    url = (
...
def compute_player_stats(match_rows):
    ...
    # Normalize to qSNER: aSNER * (15 / max_aSNER)
    max_a_sner = max(p["a_sner"] for p in players) if players else 1
    if max_a_sner == 0:
        max_a_sner = 1
    for p in players:
        p["qSNER"] = round(p["a_sner"] * (15 / max_a_sner), 4)
...
def main():
    if SHEET_ID == "YOUR_SHEET_ID_HERE":
        print("ERROR: Set GOOGLE_SHEET_ID environment variable or edit SHEET_ID in build.py", file=sys.stderr)
        sys.exit(1)

    print(f"Fetching Match History from Sheet ID: {SHEET_ID}...")
